...
export async function create(req: Request, res: Response, next: NextFunction) {
  const { title, rating, year,
    genre, pages, thumb, hasBeenRead, authorId } = req.body as BookInterface;
...
export async function update(req: Request, res: Response, next: NextFunction) {
    const { title, rating, year, genre, pages,
      thumb, hasBeenRead, authorId } = req.body as BookInterface;
    const { id } = req.params;
    const bookService = new BookService();
    try {
      if (id === undefined) {
        throw new BadRequest('Você precisa enviar o id do livro');
      }
      if (title === undefined) {
        throw new BadRequest('Você precisa enviar o título do livro');
      }
