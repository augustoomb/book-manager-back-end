...
import { ComplexModelAuthor } from './model';

const DATABASE = 'book_manager_db'

export default class AuthorModel implements ComplexModelAuthor<Author> {
    ...
    async findByName(name: string): Promise<Partial<Author> | null> {
        // const result = await this.connection.execute<ROwDataPacket[]>(`SELECT id, name, site
        const result = await this.connection.execute(`SELECT id, name, site
         FROM ${DATABASE}.${this.tableName} AS A WHERE A.name = ?;`, [name]);
